...
let rev_t (l:'a list) : 'a list =
  let rec rev_t_helper (acc:'a list) (l:'a list) : 'a list =
    begin match l with
      | [] -> acc
      | (h::tl) -> rev_t_helper (h::acc) tl
    end
  in
    rev_t_helper [] l
...
let rec lookup (x:string) (c:ctxt) : int64 =
  match c with
    | [] -> raise Not_found
    | (str, value)::tl -> if str = x then value else lookup x tl
...
let rec interpret (c:ctxt) (e:exp) : int64 =
  ...
  | Neg(e1) -> Int64.mul (interpret c e1) (-1L)
...
let rec optimize (e:exp) : exp =
  match e with
  | Mult(x1, Const 1L) | Mult(Const 1L, x1) -> x1
  | Mult(_, Const 0L)  | Mult(Const 0L, _) -> Const(0L)
  | Mult(Const x1, Const x2) -> Const(Int64.mul x1 x2)
  | Mult(x1, x2) -> optimize (Mult(optimize x1, optimize x2)) 
  | Add (x1, Const 0L) | Add (Const 0L, x1) -> x1
  | Add (Const x1, Const x2) -> Const(Int64.add x1 x2)
  | Add(x1, x2) -> optimize (Add(optimize x1, optimize x2))
  | _ -> e
