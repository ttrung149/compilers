  ...
  ("Array subtype match",
   (fun () ->
      if Typechecker.subtype
          ...
          (TRef (RArray TInt))
      then
        () else failwith "should have succeeded"));

  (* Should not succeed *)
  ("Array subtype does not match",
   (fun () ->
      if Typechecker.subtype Tctxt.empty
          (TRef (RArray TInt))
          (TRef (RArray TBool))
      then
        failwith "should have failed" else ()));
]


let manual_tests : suite = [
    GradedTest ("Posted Piazza Test Case", 5, subtype_fun_unit_tests)
