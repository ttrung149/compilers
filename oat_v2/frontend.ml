...
let oat_alloc_struct ct (id:Ast.id) : Ll.ty * operand * stream =
  ...
  let struct_id = gensym "raw_struct" in
  let ans_ty = cmp_ty ct (TRef (RStruct id)) in
  let struct_ty = Ptr I64 in
  let size = Int64.of_int (List.length (TypeCtxt.lookup id ct) * 8) in
  ans_ty, Id ans_id, lift
  [
...
let rec cmp_exp (tc : TypeCtxt.t) (c:Ctxt.t) (exp:Ast.exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Length e ->
    ...
    let _ = match arr_ty with
      ...
      | _ -> failwith "ERR: Expecting array type for length operator" in
  ...
  *)
  | Ast.NewArrInit (ty, e1, id, e2) ->
    let _, sz_op, sz_code = cmp_exp tc c e1 in
    let arr_ty, arr_op, oat_alloc_array_code = oat_alloc_array tc ty sz_op in
    let arr_id = gensym "init_new_arr_ptr" in
    let c' = Ctxt.add c arr_id (Ptr arr_ty, Id arr_id) in

    let arr_len_id = gensym "init_len" in
    let c'' = Ctxt.add c' arr_len_id (Ptr I64, Id arr_len_id) in

    let ans_id = gensym "init_new_arr_ans" in
    let arr_id_code = lift [
        (arr_id, Alloca arr_ty);
        ("", Store (arr_ty, arr_op, Id arr_id))
    ]
    in
    let arr_len_code = lift [
        (arr_len_id, Alloca I64);
        ("", Store (I64, sz_op, Id arr_len_id))
    ]
    in
    let decl = id, no_loc (CInt 0L) in
    let cond = no_loc (Bop(Lt, no_loc @@ Id id, no_loc @@ Id arr_len_id)) in
    let after = no_loc (Assn(no_loc @@ Id id,
                             no_loc @@ Bop(Add, no_loc @@ Id id, no_loc @@ CInt 1L))) in
    let body = no_loc (Assn(no_loc @@ Index(no_loc @@ Id arr_id, no_loc @@ Id id), e2)) in
    let for_s = no_loc (For([decl], Some cond, Some after, [body])) in
    let _, arr_code' = cmp_stmt tc c'' Void for_s in
    arr_ty, Id ans_id,
      sz_code >@
      oat_alloc_array_code >@
      arr_id_code >@
      arr_len_code >@
      arr_code' >@
      lift [ans_id, Load(Ptr arr_ty, Id arr_id)]
  ...
  | Ast.CStruct (id, l) ->
    ...
    let acc_code (f_code, stream) (fid, fexp) =
      let exp_ty, exp_op, exp_code = cmp_exp tc c fexp in
      let field_ptr_id = gensym "fld_ptr_id" in
      let field_val_id = gensym "fld_val_id" in
      let f_ty, _ = TypeCtxt.lookup_field_name id fid tc in
      let f_ty = cmp_ty tc f_ty in
      let f_ind = TypeCtxt.index_of_field id fid tc in
	  let code = [
        I ("", Store (f_ty, Id field_val_id, Id field_ptr_id));
        I (field_val_id, Bitcast(exp_ty, exp_op, f_ty));
        I (field_ptr_id, Gep(struct_ty, struct_op, [Const 0L; i64_op_of_int f_ind]))
      ] @ f_code in
      code, exp_code @ stream
    in

    let store_code, cmp_code = List.fold_left acc_code ([], []) l in
    let code = struct_code >@ cmp_code >@ store_code in
    struct_ty, struct_op, code
...
and cmp_exp_lhs (tc : TypeCtxt.t) (c:Ctxt.t) (e:exp node) : Ll.ty * Ll.operand * stream =
  ...
  *)
  | Ast.Proj (e, i) ->
    ...
    let struct_name = begin match struct_ty with
      | Ptr(Namedt id) -> id
      | _ -> failwith "ERR: Invalid struct name for Proj"
    end
    in
    let f_ty, f_ind = TypeCtxt.lookup_field_name struct_name i tc in
    let ret_ty = cmp_ty tc f_ty in
    let ptr_id = gensym "f_idx_id" in
    ret_ty, Id (ptr_id), struct_code >@ [
      I(ptr_id, Gep(struct_ty, struct_op, [Const 0L; Const f_ind]))
    ]
...
and cmp_stmt (tc : TypeCtxt.t) (c:Ctxt.t) (rt:Ll.ty) (stmt:Ast.stmt node) : Ctxt.t * stream =
  ...
  *)
  | Ast.Cast (typ, id, exp, notnull, null) ->
    let label_notnull = gensym "notnull" in
    let label_null = gensym "null" in
    let label_merge = gensym "merge" in
    let label_cmp =gensym "cmp_id" in
    ...
    let _, notnull_code = cmp_block tc notnull_ctxt rt notnull in
    c, exp_code
        >:: I (label_cmp, Icmp (Ne, exp_ty, exp_op, Null))
        >:: T (Cbr (Id label_cmp, label_notnull, label_null))
        >:: L label_notnull >:: E(id, Alloca exp_ty)
...
let rec cmp_gexp c (tc : TypeCtxt.t) (e:Ast.exp node) : Ll.gdecl * (Ll.gid * Ll.gdecl) list =
  ...
  | CStruct (id, cs) ->
    let greater_field_idx field_a field_b =
        TypeCtxt.index_of_field id field_a tc - TypeCtxt.index_of_field id field_b tc in
    let cs' = List.sort (fun (fa, _) (fb, _) ->  greater_field_idx fa fb) cs in

    let acc (id, cst) (es, gs) =
      let decl, gs' = cmp_gexp c tc cst in
      decl :: es, gs' @ gs
    in
    let es, globals = List.fold_right acc cs' ([], []) in
    let gid = gensym "global_struct" in
    (Ptr (Namedt id), GGid gid), (gid, (Namedt id, GStruct es)) :: globals
